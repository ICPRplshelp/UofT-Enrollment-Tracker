...
export class AppComponent {
  ...
  determineCourseCode(crsCode: string): string {
    ...
    if (!crsCode.match(/^[A-Z]{3}[A-D\d]\d{2}/))
      ...
      this.curErrorMessage =
        "You didn't provide enough information about your course, or it doesn't exist";
    return '';
  }
  ...
  private _convertToSpacesLeft(course: Course): Meeting[] {
    console.log('Checking spaces left');
    ...
    // const negativeAtOnePoint = !temp.negativeAtOnePoint;
  ...
  private _flattenUnTransposedNumericalArray2(doubleArr: number[][]): {
    ...
    for (let i = 0; i < doubleArr[0].length; i++) {
      ...
      console.log(arrayHasNonNegativeValue);
